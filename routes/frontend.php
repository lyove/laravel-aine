...
$serveUploadedFile = function (string $relativePath) {
    $real = realpath(storage_path('app/'.$relativePath));
    ...
    return response()->file($real, ['Cache-Control' => 'public, max-age=604800']);
};

Route::get('uploads/{dir}/{file}', function ($dir, $file) use ($serveUploadedFile) {
    return $serveUploadedFile('public/'.$dir.'/'.$file);
});

Route::get('uploads/thumb/{dir}/{file}', function ($dir, $file) use ($serveUploadedFile) {
    return $serveUploadedFile('public/'.$dir.'/thumbnails/'.$file);
});
