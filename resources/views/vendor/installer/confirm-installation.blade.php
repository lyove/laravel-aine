...
@section('container')
    ...
    <div class="mt-6 overflow-hidden rounded-lg ring-1 ring-slate-200 bg-white">
        ...
        <div class="px-4 py-2 bg-slate-50 border-b border-slate-100">
            <span class="text-xs font-bold uppercase tracking-wide text-slate-500">{{ trans('installer_messages.confirm.section_key') }}</span>
        </div>
        <dl class="divide-y divide-slate-100 text-sm">
            ...
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.app_debug') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['app_debug'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
            ...
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.database_host') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['database_host'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.database_port') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['database_port'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.database_username') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['database_username'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.admin_name') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['admin_name'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
        ...
        <div class="px-4 py-2 bg-slate-50 border-t border-slate-200 border-b border-slate-100">
            <span class="text-xs font-bold uppercase tracking-wide text-slate-500">{{ trans('installer_messages.confirm.section_other') }}</span>
        </div>
        <dl class="divide-y divide-slate-100 text-sm">
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.log_level') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['log_level'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.cache_driver') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['cache_driver'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.session_driver') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['session_driver'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.mail_mailer') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['mail_mailer'] ?? '-' }}</dd>
            </div>
            <div class="flex items-center justify-between px-4 py-3">
                <dt class="font-semibold text-slate-500">{{ trans('installer_messages.confirm.mail_from_address') }}</dt>
                <dd class="font-medium text-slate-900">{{ $summary['mail_from_address'] ?? '-' }}</dd>
            </div>
        </dl>
    </div>
