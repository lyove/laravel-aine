<template>
    <div class="p-5 w-full m-auto">
        <div class="flex-1 p-4" v-if="!submitSuccess">
            <h3 class="text-lg font-bold mb-2">{{ __(form.name) }}</h3>
            <h5>{{ __(form.description) }}</h5>
                        ...
                        <div v-if="field.type == 'media' && field.options.media !== undefined" class="w-full">
                            ...
                            >
                                <div class="mt-1 px-6 pt-5 pb-6 text-sm text-gray-400 group-hover:text-indigo-500">
                                    ...
                                    <span class="cursor-pointer" v-if="field.options.media.type == '1'">{{ __('Select a file or drag and drop') }}</span>
                                    <span class="cursor-pointer" v-else-if="field.options.media.type == '2'">{{ __('Select files or drag and drop') }}</span>
                                    <p class="text-xs text-gray-500" v-if="upload_max_filesize !== null">
                                        {{ __('jpeg, jpg, png, gif, webp') }}
                                        <span v-if="field.options.media.type == '2'">| {{ __('max: 4 files') }}</span>
                                        | {{ __('Up to') }} {{ $filters.prettyBytes(upload_max_filesize, 1024) }}
                                    </p>
                                </div>
                            </div>
                            <input
        ...
        <div v-else class="border p-4">
            {{ __('Your form has been submitted successfully.') }}<br />
        ...
        submitForm() {
            ...
            if (this.turnstileSiteKey && !this.turnstileToken) {
                this.newData.errors = {
                    ...this.newData.errors,
                    turnstile: [
                        __("Please complete the security check before submitting."),
                    ],
                };
