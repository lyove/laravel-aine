                ...
                <div class="w-72 flex-shrink-0 border-r border-gray-100 pr-3 overflow-y-auto max-h-[520px]">
                    ...
                    >
                        ...
                        <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                                v-if="!revision.is_current && !isReadonly"
                    ...
                    <div v-else-if="selectedRevision">
                        <div class="flex items-center justify-between mb-3 pb-2 border-b">
                            ...
                            <div class="flex items-center gap-2">
                                <button
                                    v-if="!selectedRevision.is_current && !restoring && !isReadonly"
    ...
    <ui-modal :show="showRestoreConfirm" max-width="lg" @close="showRestoreConfirm = false">
        ...
        <template #content>
            <div v-if="restoreTarget">
                ...
                <div v-if="restoreDiff" class="border rounded-lg overflow-hidden max-h-60 overflow-y-auto">
                    ...
                    <div class="divide-y">
                        <div v-for="change in restoreDiff.changes" :key="change.field" class="px-3 py-2 text-xs">
                            ...
                            <span class="text-green-700">{{ formatValue(change.new) || __("(empty)") }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="restoreFieldsToDelete.length" class="mt-3 border border-red-200 bg-red-50 rounded-lg p-3">
                    <p class="text-xs font-semibold text-red-700 mb-1">
                        <i class="fa fa-exclamation-triangle mr-1"></i>
                        {{ __('The following fields are not in this version and their data will be permanently deleted:') }}
                    </p>
                    <div class="flex flex-wrap gap-1">
                        <span
                            v-for="field in restoreFieldsToDelete"
                            :key="field"
        ...
        <template #footer>
            ...
            <ui-button color="indigo-500" :disabled="restoring || isReadonly" @click="doRestore">
    ...
    methods: {
            ...
            if (current && current.id !== revision.id) {
                ...
                axios
                    ...
                    .then((response) => {
                        ...
                        const changes = (response.data && response.data.changes) || [];
                        this.restoreFieldsToDelete = changes
        ...
        doRestore() {
            ...
            axios
                ...
                .then(() => {
                    this.$toast?.success?.(__("Revision restored successfully."));
                    this.showRestoreConfirm = false;
                    this.$emit("restored");
                    this.load();
                })
