        ...
        <div class="flex flex-1 overflow-y-auto">
            ...
            <div class="w-9/12 p-4 overflow-x-auto">
                <div v-if="collection_id !== undefined" class="admin__project-content-table">
                    ...
                    <!-- Directory card grid: listing-card browsing (differs from the default table view) -->
                    <div v-if="content.data && content.data.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div
                            v-for="item in content.data"
                            :key="item.id"
                            class="border border-gray-200 rounded-md bg-white shadow-sm hover:shadow-md transition"
                        >
                            <div class="p-4">
                                <div class="flex items-start justify-between gap-2">
                                    ...
                                    <span
                                        ...
                                        class="text-gray-500 rounded-md bg-green-200 px-2 py-0.5 text-xs whitespace-nowrap shrink-0"
                                        >{{ __('published') }}</span
                                    >
                                    <span v-else class="text-gray-500 rounded-md bg-gray-200 px-2 py-0.5 text-xs whitespace-nowrap shrink-0">{{ __('draft') }}</span>
                                </div>
                                <p class="text-sm text-gray-500 mt-2 leading-relaxed" style="display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;">
                                    {{ summaryValue(item) }}
                                </p>
                            ...
                            <div class="border-t border-gray-100 px-4 py-2 bg-gray-50 rounded-b-md flex items-center justify-between">
                                <input
                                    type="checkbox"
...
export default {
    ...
    setup() {
        ...
        // Directory card-view render helpers (UI-layer private, no effect on shared logic)
