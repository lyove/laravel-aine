            ...
            <template #emptystate>
              {{ __("No data found") }}
                    ...
                    <ui-modal maxWidth="5xl" :show="openTextModal" @close="closeTextModal">
                        <template #title>{{ textModalFieldName }}</template>
                        <template #content><div v-html="textRecord"></div></template>
                        <template #footer>
                            <ui-button color="gray-200" hover="gray-300" @click="closeTextModal"><span class="text-gray-800">{{ __('Close') }}</span></ui-button>
                        </template>
                    </ui-modal>

                    <ui-modal maxWidth="5xl" :show="openMediaModal" @close="closeMediaModal">
                        <template #title>{{ mediaModalFieldName }}</template>
                        <template #content>
                        ...
                        <template #footer>
                            <ui-button color="gray-100" hover="gray-200" @click="closeMediaModal"><span class="text-gray-800">{{ __('Close') }}</span></ui-button>
                        </template>
                    </ui-modal>

                    <ui-modal maxWidth="5xl" :show="openRelationModal" @close="closeRelationModal">
                        <template #title>{{ relationModalFieldName }}</template>
                        <template #content>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap" :class="{ 'w-full': index === 0 }" v-for="(field, index) in relationRecords.collection.fields" :key="field.id" v-show="field.type != 'richtext' && field.type != 'password' && field.type != 'media' && field.type != 'json' && field.type != 'block' && field.type != 'relation' && !safeOptions(field).hideInContentList">
                                                <div class="w-full flex justify-between item-center">{{ __(field.label) }}</div>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tr v-for="item in relationRecords" :key="item.id">
                                        <td class="pl-2 py-4 text-sm text-center w-px">
                                            <span class="text-gray-500 text-sm rounded-md bg-gray-200 px-3 py-1 whitespace-nowrap">{{ __(relationRecords.collection.name) }}</span>
                                        </td>
                                        <td class="pl-2 py-4 text-sm text-center w-px">
                                        ...
                                        <td class="px-6 py-3 text-sm w-px whitespace-nowrap text-gray-600">{{ $filters.date(item.created_at, "D MMM YYYY, H:mm") }}</td>
                                        <td class="px-6 py-3 text-sm whitespace-nowrap" :class="{ 'w-full': index === 0, 'w-auto': index !== 0 }" v-for="(field, index) in relationRecords.collection.fields" :key="field.id" v-show="field.type != 'richtext' && field.type != 'password' && field.type != 'media' && field.type != 'json' && field.type != 'block' && field.type != 'relation' && !safeOptions(field).hideInContentList">
                                            <span v-for="meta in item.meta" :key="meta.id">
                                                <span v-if="meta.field_name == field.name" :class="{ 'rounded-md bg-gray-100 p-1 mr-1': safeOptions(field).repeatable && meta.value !== null }">
                                                    <span v-if="field.type == 'date'">{{ $filters.date(meta.value) }}</span>
                                                    <span v-else-if="field.type == 'longtext' && meta.value !== null" :title="meta.value">{{ meta.value.substring(0, 20) }}<span v-if="meta.value.length > 20">...</span></span>
                        ...
                        <template #footer>
                            <ui-button color="gray-100" hover="gray-200" @click="closeRelationModal"><span class="text-gray-800">{{ __('Close') }}</span></ui-button>
                        </template>
                    </ui-modal>
                </div>
            </div>
...
export default {
    ...
    setup() {
        ...
        const onColumnToggle = (key) => {
            cl.columns.value[key] = !cl.columns.value[key];
            cl.changeColumnSettings();
        };

        const onEachChange = (value) => {
            cl.each.value = Number(value);
            cl.getContent();
        };

        const onSelectAll = () => {
            cl.selectAll.value = !cl.selectAll.value;
            cl.selectAllFn();
        };

        return { ...cl, tableColumns, tdClass, onColumnToggle, onEachChange, onSelectAll };
    },
};
</script>
