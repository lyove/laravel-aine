<template>
    ...
    >
        <h4 class="h-10 flex justify-end items-center font-bold text-lg mb-2">
            ...
            <router-link
                v-if="$route.params.col_id !== undefined && !relationSelect && !isReadonly && canProject(['owner', 'admin', 'editor'])"
                :to="{
            ...
            <router-link
                v-if="!relationSelect && collection_id !== undefined && !isReadonly && canProject(['owner', 'admin', 'editor'])"
                :to="{
            ...
            <button
                v-if="!relationSelect && collection_id !== undefined && !isReadonly && canProject(['owner', 'admin'])"
                type="button"
                class="bg-white items-center px-4 py-2 border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50 focus:outline-none transition ease-in-out duration-150 ml-2"
                    ...
                    <template #content>
                        <div class="border border-gray-200 p-3">
                            ...
                            <div v-for="field in collection.fields" :key="field.id">
                                <div class="flex items-start m-2" v-if="field.type != 'password' && field.type != 'json' && field.type != 'block'">
                                    <div class="flex items-center h-5">
                                        <input :id="field.name" v-model="columns[field.name]" type="checkbox" v-formcheckbox @click="changeColumnSettings()" />
                                    </div>
                                    <div class="ml-3 text-sm">
                                        <label :for="field.name" class="font-medium text-gray-700">{{ __(field.label) }}</label>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="overflow-x-auto sm:rounded-md">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-100">
                        <tr class="relative">
                            <th v-if="!isReadonly" scope="col" class="px-6 py-3 text-center">
                            ...
                            <th
                                ...
                                :key="field.id"
                                v-show="field.type != 'password' && field.type != 'json' && field.type != 'block' && columns[field.name]"
                        ...
                        <tr v-for="item in content.data" :key="item.id" class="relative">
                            ...
                            <td class="pl-2 py-4 text-sm text-center w-24">
                                ...
                                <div v-else>
                                    <span
                                        ...
                                        v-tooltip="__('Submitted at {date}. Form name: {name}', { date: dateFormat(item.form.created_at), name: item.form.name })"
                                        >{{ __('published') }}</span
                                    >
                                    <span
                                        v-else
                            ...
                            <td v-if="listOptions.getItems != 'trashed'" class="an__sticky right-0 px-2 py-4 text-sm w-28 whitespace-nowrap text-center shadow-sm bg-white">
                                <div class="flex items-center justify-center gap-1 py-2">
                                    ...
                                    <a v-if="!isReadonly && !isComments && canProject(['owner', 'admin'])" class="text-orange-500 p-2 px-3 rounded-md hover:bg-gray-100 cursor-pointer bg-gray-50" @click="moveToTrashContent(item)">
                                        <i class="fa fa-trash-restore"></i>
                                    </a>
                                    <a v-if="!isReadonly && isComments && canProject(['owner', 'admin', 'editor'])" class="text-orange-500 p-2 px-3 rounded-md hover:bg-gray-100 cursor-pointer bg-gray-50" @click="commentTrash(item)">
                                        <i class="fa fa-trash-restore"></i>
                                    </a>
                                </div>
                            </td>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th
                                    ...
                                    v-for="(field, index) in relationRecords.collection.fields"
                                    :key="field.id"
                                    v-show="
                                        field.type != 'richtext' &&
                                        field.type != 'password' &&
...
export default {
    ...
    methods: {
        ...
        showText(field, value) {
            ...
            this.textModalFieldName = __(field.label);
        },
        ...
        async showRelationlist(field, value) {
            ...
            await axios.post("content/get-selected-records/" + this.$route.params.project_id, { data: data }).then((response) => {
                ...
                this.relationModalFieldName = __(field.label);
                this.$forceUpdate();
            });
        },
    ...
    mounted() {
        if (this.collection_id !== undefined) {
            ...
            if (filter && ['all', 'published', 'draft', 'trashed'].includes(filter)) {
                this.listOptions.getItems = filter;
            }
            this.getContent();
        }
    },
