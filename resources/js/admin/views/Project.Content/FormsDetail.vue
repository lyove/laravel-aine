    ...
    <div class="admin__project-content-forms-detail relative h-full flex flex-col">
                ...
                <div class="grid grid-cols-6 space-x-4 h-full">
                    <div class="col-span-4 p-5 bg-white mt-2 rounded-md overflow-x-hidden">
                        <div class="flex justify-end">
                                    ...
                                    <VueDraggable :list="form.fields" v-bind="dragOptions" handle=".handle">
                                        <transition-group type="transition" class="space-y-6">
                                            <div class="bg-gray-50 p-2 w-full relative handle" v-for="field in form.fields" :key="field.id">
                                                ...
                                                <div class="mt-1 relative">
                                                   ...
                                                   <div v-if="field.type == 'url'">
                                                        <div class="mt-1 flex rounded-md shadow-sm">
                                                            ...
                                                            <input type="url" :placeholder="field.placeholder" v-forminput class="rounded-l-none" disabled />
                                                        </div>
                                                    </div>
                                                ...
                                                <i v-if="!isReadonly" class="fas fa-arrows-alt mr-4 text-gray-500 cursor-pointer absolute top-0 right-0 p-2 mr-8 handle"></i>
                                                <i v-if="!isReadonly" class="fa fa-trash-alt text-red-500 cursor-pointer absolute top-0 right-0 p-2" @click="removeField(field)"></i>
                                            </div>
                                        </transition-group>
                                    </VueDraggable>
                                </form>

                                <div v-if="!isReadonly" class="w-full text-center p-4 mt-5 border border-dashed cursor-pointer text-gray-400 hover:text-gray-500" @click="openAddFieldsModal = true">
                            ...
                            <div class="pl-4 w-52">
                                <ui-button :color="'indigo-500'" class="w-full text-center" :disabled="!isSavingEnable || isReadonly" @click="saveForm">
                                ...
                                <hr class="m-5 mt-7" />
                                <ui-button v-if="!isReadonly" :color="'red-500'" class="w-full text-center mt-2" @click="deleteForm">
        ...
        <ui-modal :show="openNewFormModal" @close="closeNewFormModal">
            ...
            <template #footer>
                ...
                <ui-button v-if="!isReadonly" color="indigo-500" @click="saveNew" :disabled="processing_new_form" class="w-40">
        ...
        <ui-modal :show="openAddFieldsModal" @close="closeAddFieldsModal" v-if="openAddFieldsModal">
            <template #title>
                {{ __('Add Fields to') }} <strong>{{ __(form.name) }}</strong>
            </template>
                    ...
                    <li class="w-full mb-2" v-for="field in collection.fields" :key="field.id">
                        ...
                        >
                            ...
                            <div class="items-center w-full">
                                <div class="text-lg">{{ __(field.label) }}</div>
...
import ContentFormsSidebar from "@/admin/components/ContentFormsSidebar.vue";
    ...
    data() {
        return {
            ...
            fieldDetails: {
                text: {
                    label: __("Text"),
                    desc: __("Single line text (headings, titles)"),
                    icon: "fas fa-font",
                    bg: "bg-yellow-400",
                },
                longtext: {
                    label: __("Long Text"),
                    desc: __("Multi line text like descriptions"),
                    icon: "fas fa-align-left",
                    bg: "bg-yellow-600",
                },
                richtext: {
                    label: __("Rich Text"),
                    desc: __("Rich text editor with formatting"),
                    icon: "fas fa-outdent",
                    bg: "bg-yellow-900",
                },
                url: {
                    label: __("URL"),
                    desc: __("Web address with validation"),
                    icon: "fas fa-globe",
                    bg: "bg-cyan-500",
                },
                slug: {
                    label: __("Slug"),
                    desc: __("Like Urls and permalinks"),
                    icon: "fas fa-link",
                    bg: "bg-green-400",
                },
                email: {
                    label: __("E-mail"),
                    desc: __("Email field with validation"),
                    icon: "fas fa-at",
                    bg: "bg-red-400",
                },
                password: {
                    label: __("Password"),
                    desc: __("Password field with encryption"),
                    icon: "fas fa-lock",
                    bg: "bg-blue-400",
                },
                number: {
                    label: __("Number"),
                    desc: __("Integer, decimal, float numbers"),
                    icon: "fas fa-sort-numeric-up",
                    bg: "bg-pink-600",
                },
                enumeration: {
                    label: __("Enumeration"),
                    desc: __("List of values"),
                    icon: "fas fa-list-ol",
                    bg: "bg-green-600",
                },
                boolean: {
                    label: __("Boolean"),
                    desc: __("True or false"),
                    icon: "fas fa-check-square",
                    bg: "bg-red-600",
                },
                color: {
                    label: __("Color"),
                    desc: __("Color picker"),
                    icon: "fas fa-tint",
                    bg: "bg-orange-400",
                },
                date: {
                    label: __("Date"),
                    desc: __("Calendar date picker"),
                ...
                media: {
                    label: __("Media"),
                    desc: __("Files, images, videos from the library"),
                    icon: "fas fa-photo-video",
                    bg: "bg-gray-500",
                },
                relation: {
                    label: __("Relation"),
                    desc: __("Collection relations"),
                    icon: "fas fa-exchange-alt",
                    bg: "bg-pink-400",
                },
                json: {
                    label: __("JSON"),
                    desc: __("Data in JSON format"),
                    icon: "fas fa-code",
                    bg: "bg-red-700",
                },
            },
