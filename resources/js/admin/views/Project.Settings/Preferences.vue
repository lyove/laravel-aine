        ...
        <div class="flex">
            ...
            <div class="w-9/12 overflow-x-hidden">
                <div class="p-4">
                    ...
                    <!-- Tab: Theme -->
                    <div v-if="activeTab === 'theme'" class="w-full bg-white rounded-md p-4">
                        <div class="grid grid-cols-1 gap-2">
                            <div>
                                <label v-formlabel>{{ __('Theme') }}</label>
                                <div class="mt-1 relative">
                                    <v-select
                                        v-model="form.admin_theme"
                                        :options="themeOptions"
                                        :get-option-key="(o) => o.value"
                                        :get-option-label="(o) => o.label"
                                        :reduce="(o) => o.value"
                                        :clearable="true"
                                        :placeholder="__('Select a theme or leave empty for default')"
                                        class="v-select"
                                    ></v-select>
                                </div>
                                <p class="text-xs text-gray-500 mt-1">{{ __('Choose the admin interface theme for this project. Different themes can provide customized UI for content management.') }}</p>
                            </div>
                            <hr class="my-2" />
                            <div>
                                <label v-formlabel></label>
...
import { getThemes } from '@/admin/viewRegistry';
...
export default {
    ...
    data() {
        ...
        return {
            ...
            form: {
                ...p,
                admin_theme: p.admin_theme || null,
    ...
    computed: {
        ...
        availableThemes() {
            ...
            return Object.entries(themes).map(([slug, config]) => ({
                slug,
        ...
        themeOptions() {
            return this.availableThemes.map(t => ({ value: t.slug, label: t.name }));
        },
        timezones() {
            let list = null;
