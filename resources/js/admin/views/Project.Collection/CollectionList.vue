<template>
    <div class="admin__project-collection-list relative h-full flex flex-col">
        ...
        <div class="flex flex-1 overflow-y-auto">
            <div class="w-3/12 bg-white overflow-x-hidden overflow-y-auto">
                <collection-sidebar :project="project"></collection-sidebar>
            </div>
            <div class="flex-1 p-4 overflow-x-auto flex flex-col min-h-0">
                <div class="mb-2 p-2 font-bold text-lg flex items-center shrink-0 bg-white z-10 border-b border-gray-100">
                    <div class="flex flex-1 min-w-0 items-center">
                        ...
                        <ui-dropdown align="left">
                            ...
                            <template #content>
                                <ui-button v-if="!isReadonly && canProject(['owner', 'admin'])" type="button" color="white" hover="gray-200" class="w-full h-full" @click="openEditCollectionModal = true">
                                ...
                                <ui-button v-if="!isReadonly && canProject(['owner', 'admin'])" type="button" color="white" hover="gray-200" class="w-full h-full" @click="openImportSchemaModal = true">
                                    <div class="text-black text-sm text-left">
                                        <i class="fa fa-upload text-green-500"></i>
                                        {{ __('Import Schema') }}
                                    </div>
                                </ui-button>
                                <ui-button v-if="!isReadonly && canProject(['owner', 'admin'])" type="button" color="white" hover="gray-200" class="w-full h-full" @click="deleteCollection(collection)">
                                    <div class="text-black text-sm text-left">
                    ...
                    <div v-if="!isReadonly && canProject(['owner', 'admin'])" class="flex items-center ml-2 shrink-0">
                        ...
                        <transition-group type="transition" class="_trans-group" :css="false">
                            <div class="w-full mb-3" v-for="field in collection.fields" :key="field.id">
                                <div class="flex items-center w-full bg-white rounded-md p-4 shadow-sm">
                                    ...
                                    <div v-if="!isReadonly && canProject(['owner', 'admin'])" class="flex items-center gap-1.5 shrink-0">
                                        <a
    ...
    data() {
        return {
            ...
            fieldDetails: {
                ...
                block: {
                    label: __("Block"),
                    desc: __("Repeatable block of sub-fields"),
