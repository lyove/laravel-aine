...
class DemoProjectsSeeder extends Seeder
{
    ...
    protected function makeGradientJpeg(string $path, int $w, int $h, array $from, array $to, string $label, bool $png = false): void
    {
        ...
        $this->drawCenteredLabel($img, $w, $h, $label, $textColor);
    ...
    /**
     * Draw the label centered on the image using a TrueType font (large,
     * crisp). Falls back to the built-in bitmap font when no TTF is found.
     *
     * @param resource $img
     */
    protected function drawCenteredLabel($img, int $w, int $h, string $label, int $color): void
    {
        $fontPath = $this->findTtfFont();

        if ($fontPath !== null) {
            // Target ~5x the old bitmap font height (15px -> ~75px).
            $maxSize = (int) round($w / 16);
            $maxWidth = (int) ($w * 0.8);
            $size = $maxSize;

            // Shrink until the label fits within 80% of the image width.
            while ($size > 12) {
            ...
            $x = (int) (($w - $textW) / 2);
            $y = (int) (($h - $textH) / 2);
            imagettftext($img, $size, 0, $x, $y, $color, $fontPath, $label);

            return;
        }

        // Fallback: built-in bitmap font (max size 5, small but always works).
        $font = 5;
        $tw = imagefontwidth($font) * strlen($label);
        imagestring($img, $font, (int) (($w - $tw) / 2), (int) (($h - imagefontheight($font)) / 2), $label, $color);
    }

    /**
     * Locate a TrueType font file across common platforms.
     */
    protected function findTtfFont(): ?string
    {
        $candidates = [
    ...
    protected function seedNoteProject(): void
    {
        ...
        $media = $this->seedMedia($project, [
            ...
            ['cover-putonghua.jpg', 'Putonghua', [13, 148, 136], [14, 165, 233], 1200, 630, false],
            ['cover-yanjiang.jpg', 'Speech', [217, 119, 6], [239, 68, 68], 1200, 630, false],
            ['cover-raokouling.jpg', 'Tongue Twister', [124, 58, 237], [236, 72, 153], 1200, 630, false],
            ['cover-jingdian.jpg', 'Classics', [37, 99, 235], [6, 182, 212], 1200, 630, false],
        ]);
        ...
        foreach ([
            ...
            ['经典语句', 'jingdian-yuju', '名言警句、诗词名句与谤语俗语，积累表达素材。'],
        ] as $i => [$title, $url, $description]) {
        ...
        /* --- Tags --- */
        $tags = [];
        foreach (['知识管理', '灵感', '速记', '会议记录', '读书笔记', '待办', '清单', '提醒', '习惯', '复盘', '模板', '标签', '归档', '搜索', '声母', '韵母', '声调', '轻声', '儿化', '多音字', '朗读', '演讲', '励志', '校园', '职场', '口才', '名言', '谤语'] as $i => $tag) {
        ...
        $postData = [
            ...
            ['读书笔记的正确打开方式', 'jishiben-dushubiji', '读完就忘？因为你只“读”不“记”。读书笔记让每本书真正变成你的。', '<h2>读书笔记三层法</h2><ol><li><strong>摘录</strong>：划线、抚金句——积累素材。</li><li><strong>转述</strong>：用自己的话概括核心观点——检验理解。</li><li><strong>关联</strong>：联系自身经历或其他书——内化知识。</li></ol><h2>工具建议</h2><p>用云笔记建一个“读书”分类，每本书一篇笔记。纸质书拍照存图，电子书直接摘抄。</p><p>关键是<strong>用自己的话写</strong>——照抄只是搬运，转述才是消化。</p>', '记事本', ['读书笔记', '知识管理'], 'cover-memo.jpg', 0, 0, 1],
            ...
            ['韵母与声调：普通话的“四声”', 'putonghua-yunmu-shengdiao', '一声平、二声扬、三声拐弯、四声降——声调是普通话的“灵魂”。', '<h2>四个声调</h2><p>普通话有四个基本声调：阴平（55）、阳平（35）、上声（214）、去声（51）。</p><h2>声调与意义</h2><p>同一个音节，声调不同意思完全不同：mā（妈）、má（麻）、mǎ（马）、mà（骂）。</p><h3>练习方法</h3><ol><li>用“ā á ǎ à”做长音练习；</li><li>把词语按声调组合朗读：山河锦绮、光明磊落。</li></ol>', '普通话学习', ['韵母', '声调'], 'cover-putonghua.jpg', 0, 1, 0],
            ...
            ['竞聘演讲稿：以实干作答', 'yanjiang-jingpin', '不喊口号、不摆姿态，用一件件实事证明自己——竞聘演讲的务实范本。', '<h2>以实干作答</h2><p>各位领导、各位同事：</p><p>大家好！今天我竞聘的岗位是项目主管。我不打算说太多漂亮话，只想用三件事回答大家。</p><p><strong>第一，我能干活。</strong>过去一年，我牵头完成了三项跨部门南作项目，全部按期交付。</p><p><strong>第二，我肯学习。</strong>我利用业余时间完成了管理课程进修，并把它用在了团队南作上。</p><p><strong>第三，我愿担责。</strong>遇到问题不推诿，先解决问题，再复盘原因。</p><p>如果大家给我这个机会，我会用行动证明：选择我，不会错。</p>', '演讲稿', ['演讲', '职场'], 'cover-yanjiang.jpg', 0, 0, 1],
            ...
            // 绕口令
            ['绕口令：四是四，十是十', 'raokouling-sishi-shishi', '数字绕口令的经典，专治平翇舌不分。', '<h2>四是四，十是十</h2><p>四是四，十是十，十四是十四，四十是四十。</p><p>谁要说十四是四十，就打谁四十；谁要说四十是十四，就罚谁十四。</p><p><strong>练习提示</strong>：先慢后快，把“四（sì）”与“十（shí）”的平翇舌读准。</p>', '绕口令', ['声母', '韵母'], 'cover-raokouling.jpg', 1, 0, 0],
            ...
            ['绕口令：打南边来了个嗇嘛', 'raokouling-lama', '嗇嘛与哑巴，提鳅目与吹嗇叭——绕口令里的“高峰挑战”。', '<h2>打南边来了个嗇嘛</h2><p>打南边来了个嗇嘛，手里提拉着五斤鳅目；打北边来了个哑巴，腰里别着个嗇叭。</p><p>南边提拉着鳅目的嗇嘛，要拿鳅目换北边别嗇叭的哑巴的嗇叭；哑巴不愿意拿嗇叭换提拉着鳅目的嗇嘛的鳅目。</p><p><strong>练习提示</strong>：这段绕口令难度较高，建议先分段慢读，再逐步加速。</p>', '绕口令', ['朗读'], 'cover-raokouling.jpg', 0, 0, 1],
            // 经典语句
            ...
            ['劵学：积跬步以至千里', 'jingdian-quanxue', '不积跬步，无以至千里——资子劵学中的坚持之道。', '<h2>资子《劵学》名句</h2><blockquote>不积跬步，无以至千里；不积小流，无以成江海。——《资子·劵学》</blockquote><blockquote>锶而舍之，条木不折；锶而不舍，金石可镯。——《资子·劵学》</blockquote><p>学习贵在积累，日拱一卒，功不唐捐。</p>', '经典语句', ['名言'], 'cover-jingdian.jpg', 0, 1, 1],
            ['中华诗词名句选', 'jingdian-shici', '从“长风破浪”到“一览众山小”，品读诗词里的豪情与境界。', '<h2>诗词名句</h2><blockquote>长风破浪会有时，直挂云帆济沧海。——李白《行路难》</blockquote><blockquote>会当凌绝顶，一览众山小。——杜画《望岳》</blockquote><blockquote>天行健，君子以自强不息。——《周易》</blockquote>', '经典语句', ['名言'], 'cover-jingdian.jpg', 0, 0, 0],
            ['谤语俗语：生活的智慧', 'jingdian-yanyu', '一句谤语，一段生活——俗语中的朴素哲理。', '<h2>谤语俗语精选</h2><ul><li>千里之行，始于足下。——《道德经》</li><li>宝剑锋从磨砺出，梅花香自苦寒来。</li><li>冰冻三尺，非一日之寒。</li><li>人心齐，泰山移。</li></ul><p>谤语短小精惫，适合晨读与朗诵练习。</p>', '经典语句', ['谤语'], 'cover-jingdian.jpg', 0, 0, 1],
        ];
        ...
        /* --- Globals --- */
        foreach ([
            ['label' => 'site-name', 'value' => '云笔记平台'],
            ['label' => 'site-description', 'value' => '云笔记、记事本与备忘录应用'],
            ['label' => 'footer-text', 'value' => '© 2026 云笔记平台 —— 基于 Aine 构建'],
        ...
        /* --- English pages --- */
        foreach ([
            ['title' => 'Home', 'slug' => 'home-en', 'content' => '<h1>Welcome to the Cloud Notes Platform</h1><p>A hub for <strong>cloud notes, notepads, memos and journals</strong> — helping you capture ideas, manage to-dos and preserve life moments efficiently.</p>'],
            ['title' => 'About Us', 'slug' => 'about-en', 'content' => '<h2>About the Cloud Notes Platform</h2><p>Powered by Aine, this platform focuses on note-taking, to-do management and knowledge organization. All content is accessible through a public API for your website or app.</p>'],
            ['title' => 'User Guide', 'slug' => 'guide-en', 'content' => '<h2>How to Use This Platform</h2><ol><li>Start with <strong>Cloud Notes</strong> to organize your knowledge and long-form writing.</li><li>Use the <strong>Notepad</strong> for quick notes and meeting records.</li><li>Manage daily tasks with <strong>Memos and Journals</strong>.</li></ol>'],
        ] as $i => $data) {
            $this->addContent($project, $c['pages'], $data, published: true, daysAgo: 20 - $i, locale: 'en');
        }

        /* --- English categories --- */
        $enCategories = [];
        foreach ([
            ['Cloud Notes', 'cloud-notes', 'Knowledge management and long-form notes — structure what you learn and think.'],
            ['Notepad', 'notepad', 'Quick notes, meeting records and reading notes — capture fast, retrieve easily.'],
            ['Memo', 'memo', 'To-do lists, reminders and habit tracking to manage your daily tasks.'],
            ['Diary', 'diary', 'Morning diaries, weekly reviews and emotion journals — reflection helps you grow.'],
            ['Putonghua Learning', 'putonghua-learning', 'Basics and practice texts for initials, finals, tones and pronunciation.'],
            ['Speeches', 'speeches', 'Sample speeches and writing methods for various occasions.'],
            ['Tongue Twisters', 'tongue-twisters', 'Classic tongue twisters to train articulation and clarity.'],
            ['Classic Quotes', 'classic-quotes', 'Famous sayings, poetry and proverbs — build your vocabulary of expression.'],
        ] as $i => [$title, $url, $description]) {
            $enCategories[$title] = $this->addContent($project, $c['categories'], ['title' => $title, 'slug' => $url, 'description' => $description], published: true, daysAgo: 19 - $i, locale: 'en')->id;
        }

        /* --- English tags --- */
        $enTags = [];
        foreach (['Knowledge Management', 'Inspiration', 'Quick Notes', 'Meeting Notes', 'Reading Notes', 'To-Do', 'Checklist', 'Reminders', 'Habits', 'Review', 'Templates', 'Tags', 'Archive', 'Search', 'Initials', 'Finals', 'Tones', 'Neutral Tone', 'Retroflex', 'Polyphones', 'Reading Aloud', 'Speech', 'Motivational', 'Campus', 'Workplace', 'Eloquence', 'Famous Quotes', 'Proverbs'] as $i => $tag) {
            $enTags[$tag] = $this->addContent($project, $c['tags'], ['tag' => $tag], published: true, daysAgo: 17 - $i, locale: 'en')->id;
        }

        /* --- English posts --- */
        $enPostData = [
            // Cloud Notes
            ['Build Your Knowledge System with Cloud Notes', 'en-cloud-notes-knowledge-system', 'From scattered information to structured knowledge — cloud notes help you build a second brain.', '<h2>Why Build a Knowledge System</h2><p>Every day we consume vast amounts of information, but most of it is forgotten. The core value of cloud notes is <strong>retention</strong> — categorizing, organizing and connecting scattered information into reusable knowledge.</p><h2>Three-Layer Structure</h2><ol><li><strong>Inbox</strong>: all new notes go here first, no filtering.</li><li><strong>Processing Zone</strong>: review regularly, add tags, file by category.</li><li><strong>Knowledge Base</strong>: mature notes are archived here, searchable anytime.</li></ol><p>The key is <strong>consistency</strong>, not perfection — write it down first, then organize gradually.</p>', 'Cloud Notes', ['Knowledge Management', 'Inspiration'], 'cover-notes.jpg', 0, 1, 0],
            ['Markdown for Better Notes', 'en-cloud-notes-markdown', 'Learn Markdown to write well-structured notes with minimal effort.', '<h2>What is Markdown</h2><p>Markdown is a lightweight markup language that lets you write headings, lists and code blocks in plain text.</p><h2>Common Syntax</h2><ul><li><code># Heading</code> — number of # determines the level</li><li><code>- Item</code> — unordered list</li><li><code>**bold**</code> — bold text</li><li><code>[link](url)</code> — hyperlink</li></ul><h3>Why It Works for Notes</h3><p>Your hands never leave the keyboard, you focus on content, and formatting is automatic. Nearly all major note apps support Markdown.</p>', 'Cloud Notes', ['Knowledge Management', 'Quick Notes'], 'cover-notes.jpg', 0, 1, 0],
            ['Note Organization: Categories vs Tags', 'en-cloud-notes-categories-tags', 'Categories are the skeleton, tags are the flesh — master both to keep notes organized.', '<h2>Categories vs Tags</h2><p><strong>Categories</strong> are tree-structured: a note belongs to one category. <strong>Tags</strong> are network-structured: a note can have multiple tags.</p><h2>Best Practices</h2><ol><li>Organize categories by <strong>domain</strong>: "Tech", "Reading", "Life".</li><li>Organize tags by <strong>topic</strong>: "Frontend", "Psychology", "Review".</li><li>Keep categories under 7; tags are unlimited but clean up regularly.</li></ol><p>One principle: <strong>categories help you find, tags help you connect.</strong></p>', 'Cloud Notes', ['Knowledge Management', 'Tags'], 'cover-notes.jpg', 0, 0, 1],
            ['From Paper to Cloud: My Migration Journey', 'en-cloud-notes-migration', 'A decade of paper journaling goes digital — what to keep and what to discard.', '<h2>Why Migrate</h2><p>Paper notes have a tactile feel, but <strong>search</strong> and <strong>sync</strong> are fatal pain points. After the third time digging through drawers for a specific entry, I decided to go digital.</p><h2>Migration Strategy</h2><ol><li><strong>Photograph for archive</strong>: scan old notebooks as a backup.</li><li><strong>Re-enter gradually</strong>: prioritize frequently referenced content.</li><li><strong>Keep the ritual</strong>: use paper for brainstorm drafts, cloud for long-term storage.</li></ol><p>Migration is not all-or-nothing — paper and cloud notes each have strengths; a hybrid approach works best.</p>', 'Cloud Notes', ['Knowledge Management', 'Inspiration'], 'cover-notes.jpg', 0, 1, 1],
            // Notepad
            ['Quick Capture: Catch Ideas in Three Seconds', 'en-notepad-quick-capture', 'Ideas are fleeting — use the shortest path to record them before they vanish.', '<h2>Why Speed Matters</h2><p>Ideas have a trait: <strong>the harder you try to remember them well, the faster they fade</strong>. Instead of polishing, just jot down keywords.</p><h2>Three-Second Rule</h2><ol><li>Open your notes app (shortcut / widget).</li><li>Write <strong>keywords</strong>, not full sentences.</li><li>Save and continue what you were doing.</li></ol><h3>Follow-up</h3><p>Spend 5 minutes each day reviewing quick notes — move valuable ones to cloud notes, delete the rest.</p>', 'Notepad', ['Inspiration', 'Quick Notes'], 'cover-memo.jpg', 1, 1, 0],
            ['Meeting Notes Template and Standards', 'en-notepad-meeting-notes', 'A good meeting record tells everyone what to do next.', '<h2>Three Elements of Meeting Notes</h2><ul><li><strong>Decisions</strong>: what conclusions were reached.</li><li><strong>Action items</strong>: who does what by when.</li><li><strong>Open issues</strong>: unresolved items for next time.</li></ul><h2>Recommended Template</h2><pre>Topic: xxx\nDate / Attendees\nDecisions:\n  1. ...\nAction items:\n  - [ ] Alice — complete A by 11/15\n  - [ ] Bob — complete B by 11/20\nOpen issues:\n  - C pending data confirmation</pre><p>Send within 10 minutes after the meeting, while memory is still fresh.</p>', 'Notepad', ['Meeting Notes', 'To-Do'], 'cover-memo.jpg', 0, 1, 0],
            ['How to Take Reading Notes Properly', 'en-notepad-reading-notes', 'Read and forget? Because you only read, not recorded. Reading notes make every book truly yours.', '<h2>Three-Layer Reading Notes</h2><ol><li><strong>Excerpt</strong>: underline and collect notable passages — build your material library.</li><li><strong>Paraphrase</strong>: summarize key points in your own words — test your understanding.</li><li><strong>Connect</strong>: relate to your own experience or other books — internalize knowledge.</li></ol><h2>Tool Tips</h2><p>Create a "Reading" category in your cloud notes. One note per book. For paper books, photograph pages; for e-books, copy directly.</p><p>The key is <strong>writing in your own words</strong> — copying is just transport, paraphrasing is digestion.</p>', 'Notepad', ['Reading Notes', 'Knowledge Management'], 'cover-memo.jpg', 0, 0, 1],
            ['Notepad vs Cloud Notes: When to Use Which', 'en-notepad-vs-cloud-notes', 'Use notepad for quick capture, cloud notes for long-term storage — do not mix them up.', '<h2>Different Purposes</h2><p><strong>Notepad</strong>: fast in, fast out. Good for temporary records — ideas, to-dos, shopping lists. Emphasis on <strong>speed</strong>.</p><p><strong>Cloud Notes</strong>: long-term storage. Good for structured knowledge — systems, reading notes, project docs. Emphasis on <strong>organization</strong>.</p><h2>How They Work Together</h2><ol><li>Idea strikes → goes to notepad first (fast).</li><li>Regular review → valuable ones move to cloud notes (organize).</li><li>Notepad items not reviewed in 7 days → delete (declutter).</li></ol>', 'Notepad', ['Quick Notes', 'Knowledge Management'], 'cover-memo.jpg', 0, 1, 0],
            // Memo
            ['How to Plan Your Daily To-Do List', 'en-memo-daily-todo-list', 'Listing tasks is not enough — a good to-do list gives you direction.', '<h2>A List Is Not a Memo</h2><p>A memo says "do not forget"; a to-do list says "what to do first today." Keep them separate.</p><h2>Planning Method</h2><ol><li><strong>Write the night before</strong>: spend 5 minutes listing the 3 most important things for tomorrow.</li><li><strong>Limit to three</strong>: TOP 3 rule — more leads to anxiety.</li><li><strong>Start with a verb</strong>: "Write report" not "Report" — make the action clear.</li></ol><h3>Execution</h3><p>Start with TOP 1 in the morning, then TOP 2. Batch small tasks in the afternoon.</p>', 'Memo', ['To-Do', 'Checklist'], 'cover-todo.jpg', 1, 0, 0],
            ['Memo Reminders: Stop Forgetting Important Things', 'en-memo-reminders', 'A good memory is no match for a pen — let alone a timely reminder.', '<h2>The Core of Reminders</h2><p>Human memory has "<strong>time blind spots</strong>" — you think you remember, but forget when the moment comes. So reminders must be <strong>written down + timed</strong>.</p><h2>Reminder Types</h2><ul><li><strong>One-time</strong>: Friday 3:00 PM meeting.</li><li><strong>Recurring</strong>: every Monday 9:00 AM weekly report.</li><li><strong>Conditional</strong>: pick up package after getting home.</li></ul><h3>Tips</h3><p>Fewer reminders are more effective — set too many and you start ignoring them. Only set reminders for truly important things.</p>', 'Memo', ['To-Do', 'Reminders'], 'cover-todo.jpg', 1, 0, 0],
            ['Habit Tracking: Build Good Habits with Memos', 'en-memo-habit-tracking', 'Checking boxes is not the goal — consistency is. Use the simplest tool to keep going.', '<h2>Why Track</h2><p>Habit formation needs <strong>visual feedback</strong> — seeing consecutive streak days makes you not want to break the chain.</p><h2>Method</h2><ol><li><strong>Track only one habit at a time</strong> — do not overreach.</li><li>Check off before bed, takes less than 10 seconds.</li><li>After 30 consecutive days, it shifts from "tracking" to "natural."</li></ol><h3>Common Pitfall</h3><p>Do not use complex apps — a memo checklist with dates is enough. The simpler the tool, the easier to persist.</p>', 'Memo', ['Habits', 'Checklist'], 'cover-todo.jpg', 0, 1, 0],
            ['Project Tracking: Manage Tasks with Kanban', 'en-memo-kanban', 'To-do, in progress, done — three columns give you a clear project overview.', '<h2>Kanban Core</h2><p>Divide tasks into three columns by status: <strong>To-Do → In Progress → Done</strong>. Each card is one task.</p><h2>Rules</h2><ul><li><strong>No more than 3 cards in progress</strong> — this limits concurrency.</li><li>Every morning, pull the highest priority from "To-Do" to "In Progress".</li><li>Move completed cards to "Done" — do not delete, seeing progress is motivating.</li></ul><p>For personal use, a memo can simulate this. For team collaboration, use a dedicated tool.</p>', 'Memo', ['Checklist', 'To-Do'], 'cover-todo.jpg', 0, 0, 1],
            // Diary
            ['Morning Diary: Start Your Day Right', 'en-diary-morning', 'Spend 5 minutes on a morning diary to start the day with clarity.', '<h2>What Is a Morning Diary</h2><p>Spend 5 minutes after waking up using a fixed template: what to do today, what to be grateful for, who you want to be.</p><h2>Recommended Template</h2><pre>Date: xxx\n\nTop 3 things today:\n  1. ...\n  2. ...\n  3. ...\n\nGratitude: ...\n\nAffirmation: Today I am a ___ person.</pre><p>Once the template is fixed, writing a diary becomes "fill in the blanks" rather than "what to write" — much less resistance.</p>', 'Diary', ['Review', 'Templates'], 'cover-diary.jpg', 0, 1, 0],
            ['Weekly Review Template: Reflect to Grow Faster', 'en-diary-weekly-review', 'A week without review is a week wasted — 5 questions to help you reflect.', '<h2>Why Weekly Reviews</h2><p>After a busy week without summarizing, experience scatters. Weekly reviews turn <strong>experience into insight</strong>.</p><h2>Five Questions</h2><ol><li>What did I accomplish this week?</li><li>What went well?</li><li>What could be improved?</li><li>What did I learn?</li><li>What are the top 3 things for next week?</li></ol><p>Spend 15 minutes every Sunday evening answering these. After a month, you will see a difference.</p>', 'Diary', ['Review', 'Templates'], 'cover-diary.jpg', 0, 1, 1],
            ['Emotion Diary: Understand Your Inner World', 'en-diary-emotion', 'Writing itself is therapeutic — an emotion diary helps you understand yourself.', '<h2>What Is an Emotion Diary</h2><p>No literary skill needed — just <strong>record honestly</strong>: what happened, what I felt, where my body was uncomfortable.</p><h2>How to Write</h2><pre>Event: xxx\nEmotion: anxious/angry/sad...\nBody: chest tight/sweaty palms\nThought: I feel xxx\n\nAsk yourself: Is this thought 100% true?</pre><p>After writing, you will find that <strong>naming the emotion</strong> alone reduces its intensity.</p>', 'Diary', ['Review', 'Inspiration'], 'cover-diary.jpg', 0, 0, 0],
            ['Travel Diary: Preserve Stories from the Road', 'en-diary-travel', 'Photos fade, words endure — a travel diary keeps every journey traceable.', '<h2>Why Write</h2><p>After returning, photos pile up in albums, but <strong>the feelings of that moment</strong> can only be captured in words.</p><h2>Key Points</h2><ul><li><strong>Write the same day</strong>: even just 3 sentences.</li><li><strong>Write feelings</strong>, not guides: guides are online; feelings are uniquely yours.</li><li><strong>Attach photos</strong>: a picture is worth a thousand words.</li></ul><p>Create a "Travel" category in your cloud notes. One entry per trip. Looking back years later is a treasure.</p>', 'Diary', ['Inspiration', 'Archive'], 'cover-diary.jpg', 0, 0, 1],

            // Putonghua Learning
            ['Initials: b p m f', 'en-putonghua-initials-bpmf', 'Bilabial and labiodental sounds with contrastive practice to build your Putonghua foundation.', '<h2>Bilabial b and p</h2><p><strong>b</strong> is an unaspirated voiceless consonant; <strong>p</strong> is aspirated. Practice with your palm in front of your mouth: barely feel airflow for b, strong airflow for p.</p><h2>Nasal m and Labiodental f</h2><p><strong>m</strong>: lips closed, airflow through nose. <strong>f</strong>: upper teeth touch lower lip, airflow squeezed through.</p><h3>Contrast Practice</h3><ul><li>baba (bà ba) — climbing (pá pō)</li><li>mama (mā ma) — abundant (fēng fù)</li></ul>', 'Putonghua Learning', ['Initials'], 'cover-putonghua.jpg', 0, 1, 0],
            ['Finals and Tones: The Four Tones', 'en-putonghua-finals-tones', 'Level, rising, dipping, falling — tones are the soul of Putonghua.', '<h2>Four Tones</h2><p>Putonghua has four basic tones: high-level (55), rising (35), dipping (214), falling (51).</p><h2>Tone and Meaning</h2><p>The same syllable with different tones means completely different things: ma (mother), ma (hemp), ma (horse), ma (scold).</p><h3>Practice Method</h3><ol><li>Practice long tones with "ā á ǎ à".</li><li>Read words by tone combinations.</li></ol>', 'Putonghua Learning', ['Finals', 'Tones'], 'cover-putonghua.jpg', 0, 1, 0],
            ['Neutral Tone and Retroflex: Sound More Natural', 'en-putonghua-neutral-retroflex', 'Neutral tone is not just "soft" and retroflex is not "optional" — master the rules for natural Putonghua.', '<h2>What Is Neutral Tone</h2><p>Neutral tone is a short, light tone change, like the second syllable of "baba" "mama", and particles like "de", "de", "de".</p><h2>What Is Retroflex</h2><p>Retroflex adds a tongue-curling action after the final, like "xiaohair" (little child), "yidianr" (a little).</p><h3>Common Neutral Tone Words</h3><p>luobo (radish), doufu (tofu), erduo (ear), shiqing (thing), pengyou (friend).</p>', 'Putonghua Learning', ['Neutral Tone', 'Retroflex'], 'cover-putonghua.jpg', 0, 0, 1],
            ['Common Polyphones', 'en-putonghua-polyphones', 'One character, multiple readings — get a polyphone wrong and the meaning changes entirely.', '<h2>Why Polyphones Exist</h2><p>Polyphones have different pronunciations based on different meanings — a key difficulty in Putonghua.</p><h3>Common Polyphones</h3><ul><li><strong>好</strong>: hǎo (good) / hào (like)</li><li><strong>行</strong>: xíng (walk) / háng (bank)</li><li><strong>长</strong>: cháng (length) / zhǎng (grow)</li><li><strong>重</strong>: zhòng (heavy) / chóng (repeat)</li></ul><p>Memorize in context — do not memorize pronunciations in isolation.</p>', 'Putonghua Learning', ['Polyphones'], 'cover-putonghua.jpg', 0, 1, 1],
            // Speeches
            ['Motivational Speech: Run Toward the Light', 'en-speech-motivational-light', 'Rather than waiting in the dark, run toward the light. A speech about perseverance and courage.', '<h2>Run Toward the Light</h2><p>Dear teachers, dear classmates:</p><p>Hello everyone! Today I want to talk about one word — <strong>running</strong>.</p><p>Life has its lows, like traveling at night. Some stop and complain about the dark; others light a torch and keep going. Light is not found — it is created by running.</p><p>Every practice, every comeback after failure, is paving lamps for your road. Trust that when you decide to start, the hardest moment has already passed.</p><p>May we all run toward the light.</p>', 'Speeches', ['Speech', 'Motivational'], 'cover-yanjiang.jpg', 1, 1, 0],
            ['Graduation Speech: Our Burning Youth', 'en-speech-graduation-youth', 'As the graduation bell rings, send the best wishes to everyone setting off on new journeys.', '<h2>Our Burning Youth</h2><p>Dear teachers, dear classmates:</p><p>Hello everyone! Today we stand at the threshold of graduation, looking back at our burning youth.</p><p>The sound of reading in classrooms, the sweat on the field, the late-night encouragement among friends — these fragments form our most precious memories.</p><p>Graduation is not a period, but a new comma. May we set off with courage and shine in our own worlds.</p>', 'Speeches', ['Speech', 'Campus'], 'cover-yanjiang.jpg', 0, 1, 0],
            ['Competitive Speech: Answer with Action', 'en-speech-competitive', 'No slogans, no posturing — prove yourself with real results. A pragmatic speech for job competition.', '<h2>Answer with Action</h2><p>Dear leaders, dear colleagues:</p><p>Hello everyone! Today I am competing for the project manager position. I will not say pretty words — just answer with three things.</p><p><strong>First, I deliver.</strong> Over the past year, I led three cross-department projects, all delivered on time.</p><p><strong>Second, I learn.</strong> I completed a management course in my spare time and applied it to team collaboration.</p><p><strong>Third, I take responsibility.</strong> When problems arise, I solve them first, then review causes.</p><p>If you give me this opportunity, I will prove it with action.</p>', 'Speeches', ['Speech', 'Workplace'], 'cover-yanjiang.jpg', 0, 0, 1],
            ['Impromptu Speech: Opening and Closing Techniques', 'en-speech-impromptu-techniques', 'A good opening grabs attention in three seconds; a good closing makes them remember you.', '<h2>Opening: Grab Attention in Three Seconds</h2><ul><li><strong>Question</strong>: Have you ever wondered...</li><li><strong>Story</strong>: Last week something happened to me...</li><li><strong>Data</strong>: According to statistics...</li></ul><h2>Closing: Make Them Remember You</h2><p>Condense your core message into one sentence, then close with an action or wish: "Thank you, and may we all become who we want to be."</p><p><em>The core: think "what does the audience want" before "what do I want to say".</em></p>', 'Speeches', ['Speech', 'Eloquence'], 'cover-yanjiang.jpg', 0, 1, 0],
            // Tongue Twisters
            ['Tongue Twister: Four is Four', 'en-tongue-twister-four-ten', 'A classic number tongue twister for practicing s and sh distinction.', '<h2>Four is Four, Ten is Ten</h2><p>Four is four, ten is ten, fourteen is fourteen, forty is forty.</p><p>Whoever says fourteen is forty, gets hit forty times; whoever says forty is fourteen, gets penalized fourteen times.</p><p><strong>Practice tip</strong>: Start slow, focus on the s (sì) and sh (shí) distinction.</p>', 'Tongue Twisters', ['Initials', 'Finals'], 'cover-raokouling.jpg', 1, 0, 0],
            ['Tongue Twister: Eating Grapes', 'en-tongue-twister-grapes', 'A tongue twister for practicing b and p bilabial sounds.', '<h2>Eating Grapes</h2><p>Eat grapes without spitting grape skins; do not eat grapes, yet spit grape skins.</p><p><strong>Practice tip</strong>: Focus on the aspiration difference between b and p. Go from slow to fast, keeping each syllable clear.</p>', 'Tongue Twisters', ['Initials', 'Reading Aloud'], 'cover-raokouling.jpg', 1, 0, 0],
            ['Tongue Twister: Shoulder Pole and Bench', 'en-tongue-twister-pole-bench', 'Long and short, wide and narrow — a classic contrast exercise.', '<h2>Shoulder Pole and Bench</h2><p>The shoulder pole is long, the bench is wide; the bench is not as long as the shoulder pole, the shoulder pole is not as wide as the bench.</p><p>The shoulder pole wants to tie onto the bench, but the bench refuses to let the shoulder pole tie onto it.</p><p><strong>Practice tip</strong>: Pay attention to the finals of "long" (cháng) and "wide" (kuān).</p>', 'Tongue Twisters', ['Initials', 'Finals'], 'cover-raokouling.jpg', 0, 1, 0],
            ['Tongue Twister: The Lama and the Mute', 'en-tongue-twister-lama', 'A high-difficulty tongue twister — the peak challenge.', '<h2>The Lama and the Mute</h2><p>A lama came from the south carrying five pounds of snails; a mute came from the north with a trumpet.</p><p>The lama with snails wanted to trade snails for the mute is trumpet; the mute refused to trade the trumpet for the lama is snails.</p><p><strong>Practice tip</strong>: This is high difficulty — practice slowly in segments first, then gradually speed up.</p>', 'Tongue Twisters', ['Reading Aloud'], 'cover-raokouling.jpg', 0, 0, 1],
            // Classic Quotes
            ['Analects: On Learning', 'en-classic-analects', 'Learn and practice often — is it not a pleasure? Wisdom from the Analects.', '<h2>Analects on Learning</h2><blockquote>To learn and practice what you have learned — is it not a pleasure? To have friends come from afar — is it not a joy? — Analects, Xue Er</blockquote><blockquote>Learning without thinking is vain; thinking without learning is dangerous. — Analects, Wei Zheng</blockquote><blockquote>Know what you know, know what you do not know — that is knowledge. — Analects, Wei Zheng</blockquote><p>Read repeatedly to appreciate the rhythm and meaning of these classic sayings.</p>', 'Classic Quotes', ['Famous Quotes'], 'cover-jingdian.jpg', 0, 1, 0],
            ['Xunzi: Accumulate Steps to Reach a Thousand Miles', 'en-classic-xunzi', 'Without accumulating steps, you cannot reach a thousand miles — the way of perseverance in Xunzi.', '<h2>Quotes from Xunzi</h2><blockquote>Without accumulating steps, one cannot reach a thousand miles; without accumulating small streams, one cannot form a river or sea. — Xunzi, Quan Xue</blockquote><blockquote>Carve and give up, even rotten wood will not break; carve and persist, even metal and stone can be engraved. — Xunzi, Quan Xue</blockquote><p>Learning is about accumulation — one step a day, effort is never wasted.</p>', 'Classic Quotes', ['Famous Quotes'], 'cover-jingdian.jpg', 0, 1, 1],
            ['Classical Chinese Poetry', 'en-classic-poetry', 'From "riding the wind and waves" to "viewing all mountains as small" — savor the spirit of poetry.', '<h2>Poetry Quotes</h2><blockquote>A time will come to ride the wind and cleave the waves; I will set my cloud-white sail and cross the sea. — Li Bai, Hard Roads</blockquote><blockquote>When I reach the summit, all mountains appear small. — Du Fu, Gazing at the Mountain</blockquote><blockquote>Heaven moves with vigor — the noble one strives ceaselessly. — Book of Changes</blockquote>', 'Classic Quotes', ['Famous Quotes'], 'cover-jingdian.jpg', 0, 0, 0],
            ['Proverbs: Wisdom of Life', 'en-classic-proverbs', 'A proverb, a slice of life — simple philosophy from folk wisdom.', '<h2>Selected Proverbs</h2><ul><li>A journey of a thousand miles begins with a single step. — Dao De Jing</li><li>The sword is sharpened on the stone; the fragrance of plum blossoms comes from the bitter cold.</li><li>Rome was not built in a day.</li><li>When hearts are united, mountains can be moved.</li></ul><p>Proverbs are short and powerful — perfect for morning reading and recitation practice.</p>', 'Classic Quotes', ['Proverbs'], 'cover-jingdian.jpg', 0, 0, 1],
        ];

        foreach ($enPostData as $i => [$title, $url, $excerpt, $content, $cat, $tagNames, $cover, $slider, $featured, $recommended]) {
            $tagIds = array_map(fn ($t) => $enTags[$t], $tagNames);
            $this->addContent($project, $c['posts'], [
                'title' => $title, 'slug' => $url, 'excerpt' => $excerpt, 'content' => $content,
                'featured-image' => $media[$cover], 'category' => $enCategories[$cat],
                'tags' => implode(',', $tagIds),
                'slider' => $slider, 'featured' => $featured, 'recommended' => $recommended,
            ], published: true, daysAgo: max(16 - $i, 1), locale: 'en')->id;
        }

        /* --- English globals --- */
        foreach ([
            ['label' => 'site-name', 'value' => 'Cloud Notes Platform'],
            ['label' => 'site-description', 'value' => 'Cloud notes, notepad and memo app'],
            ['label' => 'footer-text', 'value' => '© 2026 Cloud Notes Platform — built with Aine'],
            ['label' => 'support-email', 'value' => 'support@note.example'],
        ] as $i => $data) {
            $this->addContent($project, $c['globals'], $data, published: true, daysAgo: 15 - $i, locale: 'en');
        }
