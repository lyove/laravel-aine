...
class CommentsController extends Controller
{
    ...
    public function index(Request $request, string $projectIdentifier, int $articleId): JsonResponse
    {
        ...
        if (! $article) {
            return $this->apiError(404, 'Article not found');
        }

        $collection = $this->commentsCollection($project);
        if (! $collection) {
            return $this->apiError(404, 'Comments are not enabled for this project');
        }
        ...
        $comments = Content::query()
            ...
            ->orderBy('created_at')
            ->get()
            ->filter(fn (Content $comment) => $this->metaValue($comment, 'article') === (string) $article->id)
            ->values();
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($mode === self::MODERATION_DISABLED) {
            return $this->apiError(422, 'Comments are disabled for this article');
        }
        ...
        return response()->json([
            'success' => true,
            'code' => 201,
            'message' => $autoApproved ? null : 'Comment submitted and awaiting moderation',
            'data' => [
                'id' => $comment->id,
                'status' => $autoApproved ? 'approved' : 'pending',
            ],
