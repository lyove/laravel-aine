...
class ContentController extends Controller
{
    ...
    public function restoreRevision($project_id, $collection_id, $content_id, $revision_id, Request $request)
    {
        $project = Project::findOrFail($project_id);
        $this->authorizeEditor($project);
        ...
        $fieldsToDelete = ContentMeta::where('content_id', $content->id)
            ->pluck('field_name')
            ->diff(array_keys($snapshot))
            ->values()
            ->all();

        if ($fieldsToDelete !== [] && ! $request->boolean('confirm')) {
            return response()->json([
                'success' => false,
                'code' => 409,
