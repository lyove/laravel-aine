...
class CollectionFieldsController extends Controller
{
    ...
    public function store($project_id, $collection_id, Request $request){
        ...
        $request->validate([
            ...
            'options.enumeration.required_if' => __("Add at least one option to enumeration list"),
            'options.relation.collection.required_if' => __("Select a collection"),
            'validations.charcount.type.required' => __("This field is required"),
            'validations.charcount.min.numeric' => __("Must be numeric"),
            'validations.charcount.max.numeric' => __("Must be numeric"),
        ]);
        ...
        if($req_charcount['status']){
            if($req_charcount['type'] == 'Between'){
                if(!isset($req_charcount['min']) || $req_charcount['min'] == "" || $req_charcount['min'] == null){
                    $charcountErrors['errors']['validations.charcount.min'] = [__('Enter a value')];
                }
                if(!isset($req_charcount['max']) || $req_charcount['max'] == "" || $req_charcount['max'] == null){
                    $charcountErrors['errors']['validations.charcount.max'] = [__('Enter a value')];
                }
                if(isset($req_charcount['min']) && isset($req_charcount['max'])){
                    if($req_charcount['min'] > $req_charcount['max']){
                        $charcountErrors['errors']['validations.charcount.min'] = [__('Must be less than max')];
                    }
                }
