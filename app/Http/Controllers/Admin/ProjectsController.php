...
class ProjectsController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($templateId && $template = ProjectTemplates::get($templateId)) {
            ...
            // Auto-assign admin theme based on template slug
            $templateModel = ProjectTemplate::find($templateId);
            if ($templateModel && $templateModel->slug) {
                $project->update(['admin_theme' => $templateModel->slug]);
            }
        }
    ...
    public function update($id, Request $request){
        ...
        $themeSlugs = ProjectTemplate::query()->pluck('slug')->filter()->values()->all();
        if ($themeSlugs) {
            $rules['admin_theme'] = ['nullable', 'string', 'max:60', Rule::in($themeSlugs)];
        }
        ...
        $request->validate($rules, [
            ...
            'slug.regex' => __('Slug can only contain lowercase letters, numbers, and hyphens'),
            'slug.unique' => __('Slug already exists'),
            'admin_theme.in' => __('Selected admin theme is not available'),
        ]);
        ...
        $project->update([
            ...
            'admin_theme' => $request->get('admin_theme', $project->admin_theme),
