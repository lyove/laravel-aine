...
class Media extends Model
{
    ...
    public function getFullUrlAttribute($value) {
        ...
        if($this->disk === 'oss' || $this->disk === 'cos' || $this->disk === 'qiniu') {
            return MediaStorage::disk($this->disk)->url($uuid.'/'.$this->name);
        } elseif($this->disk === 's3') {
            return MediaStorage::disk('s3')->url('public/'.$uuid.'/'.$this->name);
        } elseif($this->disk === 'local' || $this->disk === 'public'){
    ...
    public function getFullUrlThumbAttribute($value) {
        $uuid = $this->projectUuid();
        if($this->disk === 'oss' || $this->disk === 'cos' || $this->disk === 'qiniu') {
            return MediaStorage::disk($this->disk)->url($uuid.'/thumbnails/'.$this->name);
        } elseif($this->disk === 's3') {
            return MediaStorage::disk('s3')->url('public/'.$uuid.'/thumbnails/'.$this->name);
        } elseif($this->disk === 'local' || $this->disk === 'public'){
