...
### 用户、角色与权限
- **全局角色**：每个账号为 `super_admin`（平台全部权限）或普通 `user`（无全局后台权限）。
...
### 前端
- 前台 SPA（Vue 3）挂载在 `/`，为内容站点提供现成页面：CMS 模板（首页、内容归档、分类、标签、文章详情）、黄页模板（商家列表、分类、标签、位置、点评）与笔记模板（帖子、页面、分类、标签）——全部数据通过内容 API 加载。
...
| 认证 | 后台会话认证（全局 + 项目级角色、两步验证）；内容 API 使用 Sanctum 个人访问令牌 |
...
| 前端面板 | `resources/js/admin/views/components/RevisionsModal.vue` —— 双栏布局：版本列表 + 预览 + 两版本对比 + 标签编辑 + 带预览确认的恢复 |
...
测试套件覆盖内容增删改查、版本历史（创建 / 更新 / 列表 / 对比 / 恢复 / 标签）、导入导出、媒体、API 认证与查询、工作流审计、集合字段管理、用户与权限、评论与审核以及前台首页响应。当前基线：**270 个测试 / 812 个断言**。
