...
### 方式 3：前台用户互动接口（Session 认证）

前台注册用户（角色 `user`）可对**已发布**内容执行收藏、点赞、评论，并查看自己的个人中心数据。
认证方式为 Session Cookie（`web` + `auth:web`），需先通过下方认证接口登录；未登录调用写接口返回 `401`。

#### 🔐 前台用户认证（Web 页面，非 JSON API）

| # | 方法 | 接口路径 | 说明 | 认证 |
| -- | --- | --- | --- | --- |
| 23 | GET | `/register` | 注册页面 | 游客 |
| 24 | POST | `/register` | 提交注册（限流 6 次/分钟） | 游客 |
| 25 | GET | `/login` | 登录页面 | 游客 |
| 26 | POST | `/login` | 提交登录 | 游客 |
| 27 | POST | `/logout` | 退出登录 | 已登录 |
| 28 | GET | `/forgot-password` / POST `/forgot-password` | 找回密码（限流 6 次/分钟） | 游客 |
| 29 | GET | `/reset-password/{token}` / POST `/reset-password` | 重置密码（限流 5 次/分钟） | 游客 |

#### ⭐ 收藏 & 点赞（方式 1，需登录）

| # | 方法 | 接口路径 | 说明 | 参数 | 认证 |
| -- | --- | --- | --- | --- | --- |
| 30 | POST | `/api/project/{project_identifier}/favorites` | 收藏内容（幂等，重复调用不产生重复记录） | Body: `content_id`: int（必填，须为已发布内容） | ✅ 登录用户 |
| 31 | DELETE | `/api/project/{project_identifier}/favorites/{content_id}` | 取消收藏（幂等） | `content_id`: int | ✅ 登录用户 |
| 32 | POST | `/api/project/{project_identifier}/likes` | 点赞内容（幂等） | Body: `content_id`: int（必填，须为已发布内容） | ✅ 登录用户 |
| 33 | DELETE | `/api/project/{project_identifier}/likes/{content_id}` | 取消点赞（幂等） | `content_id`: int | ✅ 登录用户 |
| 34 | GET | `/api/project/{project_identifier}/interactions/{content_id}` | 获取互动状态：收藏数、点赞数、当前用户是否已收藏/点赞（游客返回计数，`is_*` 为 false） | `content_id`: int | 无需登录 |

以上操作均限流（`throttle:api-write`）；`favorites`/`likes` 表对 `(user_id, project_id, content_id)` 建有唯一索引，从数据库层面保证幂等。

#### 💬 评论（方式 1）

| # | 方法 | 接口路径 | 说明 | 参数 | 认证 |
| -- | --- | --- | --- | --- | --- |
| 35 | GET | `/api/project/{project_identifier}/comments/{article_id}` | 获取某内容已通过审核的评论（按时间正序） | `article_id`: int；`limit`/`offset`（可选分页，`limit` 上限 100） | 无需登录 |
| 36 | POST | `/api/project/{project_identifier}/comments` | 提交评论 | Body: `article_id`: int（必填）、`comment`: string（必填，≤2000 字） | ✅ 登录用户（限流） |

评论是否立即展示取决于内容的 `comments_moderation` 设置：`auto`（默认，立即通过）/ `approval`（待审核）/ `disabled`（拒绝，返回 422）。

#### 👤 个人中心（Session 认证，仅返回本人数据）

| # | 方法 | 接口路径 | 说明 | 认证 |
| -- | --- | --- | --- | --- |
| 37 | GET | `/api/auth/me` | 当前登录用户信息（未登录返回 null） | 无需登录 |
| 38 | GET | `/api/me/profile` | 个人资料 + 收藏/点赞/评论计数 | ✅ 登录用户 |
| 39 | POST | `/api/me/avatar` | 上传/替换头像（`avatar`: 图片 ≤2MB） | ✅ 登录用户（限流） |
| 40 | GET | `/api/me/favorites` | 我的收藏列表（跨项目，最新优先，≤200 条，含标题/slug/项目/集合/发布状态） | ✅ 登录用户 |
| 41 | GET | `/api/me/likes` | 我的点赞列表（同上） | ✅ 登录用户 |
| 42 | GET | `/api/me/comments` | 我的评论列表（同上，含关联文章信息） | ✅ 登录用户 |

> 注：评论接口目前仅支持 CMS 项目的 `articles` 集合（`Directory` 的 `listings`、`Note` 的 `posts` 暂无评论接口与评论 UI；`Directory` 内容自带独立的 reviews 点评集合）。如需扩展为全内容类型通用评论，可在 `CommentsController::findArticle` 中放开集合限制。
...
// → GET /api/project/directory/listings/42/reviews?sort=created_at:desc&limit=10
```
