...
class TwoFactorTest extends TestCase
{
    ...
    // ── API token login must respect 2FA ──────────────────────────────

    public function test_api_login_without_2fa_issues_token(): void
    {
        $response = $this->postJson('/api/login', [
            'account' => 'admin@2fa.test',
            'password' => 'password',
        ]);
    ...
    public function test_api_login_with_2fa_requires_code(): void
    {
        $this->enableTwoFactorDirectly();
    ...
    public function test_api_login_with_2fa_rejects_invalid_code(): void
    {
        $this->enableTwoFactorDirectly();
    ...
    public function test_api_login_with_2fa_accepts_valid_totp_code(): void
    {
        $this->enableTwoFactorDirectly();

        $code = TwoFactor::codeAt($this->user->two_factor_secret, (int) floor(time() / 30));
    ...
    public function test_api_login_with_2fa_accepts_recovery_code(): void
    {
        $this->enableTwoFactorDirectly();

        $code = $this->user->twoFactorRecoveryCodes()[0];

        $response = $this->postJson('/api/login', [
            'account' => 'admin@2fa.test',
            'password' => 'password',
            'two_factor_recovery_code' => $code,
        ]);

        $response->assertOk()
            ->assertJsonPath('data.access_token', fn ($token) => is_string($token) && $token !== '');

        // Recovery code is consumed, as in the web flow.
        $this->user->refresh();
        $this->assertNotContains($code, $this->user->twoFactorRecoveryCodes());
    }
