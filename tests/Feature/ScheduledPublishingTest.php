...
class ScheduledPublishingTest extends TestCase
{
    ...
    {
        $owner = User::create(['name' => 'Owner', 'email' => 'owner@test.local', 'password' => bcrypt('password')]);
        $this->project->owner_id = $owner->id;
        $this->project->save();
        ...
        // The publish is attributed to the project owner and recorded in the
        // content history so the audit trail stays complete for schedules.
        $this->assertSame($owner->id, $content->published_by);
