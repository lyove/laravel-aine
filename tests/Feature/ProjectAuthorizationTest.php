...
class ProjectAuthorizationTest extends TestCase
{
    ...
    private function makeUser(string $email = 'user@test.local'): User
    {
        ...
        // These tests exercise the project-level matrix, so the actor must be
        // able to enter the admin area first (global backend role).
    ...
    public function test_editor_can_create_project_and_becomes_owner(): void
    {
        $user = $this->makeUser();
        $this->actingAs($user);
    ...
    public function test_plain_user_cannot_create_project(): void
    {
        $user = User::create(['name' => 'Plain', 'email' => 'plain@test.local', 'password' => bcrypt('password')]);
