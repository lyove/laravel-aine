...
class ContentRevisionTest extends TestCase
{
    ...
    public function test_restore_revision_rolls_back_meta(): void
    {
        ...
        $response = $this->newController()->restoreRevision(
            $project->id, $collection->id, $content->id, $firstRevision->id,
    ...
    public function test_restore_removes_fields_missing_from_snapshot(): void
    {
        ...
        // Without an explicit confirmation the restore must be refused and
        // the data must stay intact.
        $refused = $this->newController()->restoreRevision(
            $project->id, $collection->id, $content->id, $firstRevision->id,
            Request::create('/x', 'POST')
        );
        $this->assertSame(409, $refused->getStatusCode());
        $this->assertSame(['extra'], $refused->getData()->data->fields_to_delete);
        $this->assertDatabaseHas('content_meta', ['content_id' => $content->id, 'field_name' => 'extra']);

        // With confirmation the restore proceeds and the orphan field is removed.
        $this->newController()->restoreRevision(
            $project->id, $collection->id, $content->id, $firstRevision->id,
            Request::create('/x', 'POST', ['confirm' => true])
        );
